...
int main(int argc, char *argv[]){
    ...
    while(getline(in1,line)){
        ...
        while(ss>>word){
            if(find(types.begin(),types.end(),word)==types.end()) vline.push_back(word);
            else {
                type = word;
                value = vline.back();
                time = value.substr(value.length()-time.length());
                vline.back() = value.substr(0,value.length()-time.length());
                for(int i{};i<vline.size();i++) out1<<vline[i]<<'\t'; out1<<endl;
                vline.clear();
                vline.push_back(time);
                vline.push_back(type);
            }
    ...
    while(getline(in1,line)){
        ...
        if(vline[1]=="TYPE_ACCELEROMETER"){
            ...
            y_acce += vline[3] + " ";
            z_acce += vline[4] + " ";
            if(vline.size()==6) a_acce += vline[5] + " ";
            ...
            y_gyro += vline[3] + " ";
            z_gyro += vline[4] + " ";
            if(vline.size()==6) a_gyro += vline[5] + " ";
            ...
            y_magn += vline[3] + " ";
            z_magn += vline[4] + " ";
            if(vline.size()==6) a_magn += vline[5] + " ";
            ...
            y_ahrs += vline[3] + " ";
            z_ahrs += vline[4] + " ";
            if(vline.size()==6) a_ahrs += vline[5] + " ";
    ...
    out1<<pathid<<','<<start<<','<<xyz_time<<','<<x_acce<<','<<y_acce<<','<<z_acce<<','<<a_acce<<','<<
    ...
    t1_wifi<<','<<ssid_wifi<<','<<bssid_wifi<<','<<rssid_wifi<<','<<freq_wifi<<','<<t2_wifi<<','<<
    t1_beac<<','<<id_beac<<','<<power_beac<<','<<rssi_beac<<','<<dist_beac<<','<<mac_beac<<','<<t2_beac<<','<<
    t_waypoint<<','<<floor<<','<<x_waypoint<<','<<y_waypoint<<endl;
    in1.close();
    out1.close();
    return(0);
}
